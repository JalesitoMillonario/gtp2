...
export default function CoursePage() {
  ...
  const { data: lessons = [], isLoading } = useQuery({
    queryKey: ["lessons"],
    queryFn: async () => await customApi.lessons.list("order"),
    initialData: [],
    retry: 2,
    staleTime: 30000,
  });
  ...
  const { data: progress = [] } = useQuery({
    ...
    queryFn: async () => (user ? await customApi.progress.list(user.email) : []),
    enabled: !!user,
    initialData: [],
  });
  ...
  const markCompletedMutation = useMutation({
    mutationFn: async ({ lessonId, completed }) => {
      ...
      const payload = {
        ...
        lesson_id: lessonId,
        completed,
        progress_percentage: completed ? 100 : existing?.progress_percentage || 0,
        last_watched: new Date().toISOString()
      };
      if (existing) return customApi.progress.update(existing.id, payload);
      else return customApi.progress.create(payload);
    },
    onSuccess: () => queryClient.invalidateQueries(["progress", user?.email]),
  });
  ...
  const getModuleTitle = (moduleId) => ({
    ...
    proyecto_1: "Proyecto 1: IA en el Borde",
    proyecto_2: "Proyecto 2: Servidor con GPU",
  }[moduleId] || moduleId);
            ...
            <div className="lg:col-span-2">
              {selectedLesson && (
                <VideoPlayer
                  ...
                  onProgressUpdate={(p) =>
                    updateProgressMutation.mutate({ lessonId: selectedLesson.id, percentage: p })
                  }
                  onComplete={() =>
                    markCompletedMutation.mutate({ lessonId: selectedLesson.id, completed: true })
                  }
                />
              )}
            </div>
